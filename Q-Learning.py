...
class Agent:
    ...
    def act(self, state, legal_moves, q_learn=False):
        row, col = state[0], state[1]
        choice = np.random.rand()
        q_values_for_legal_moves = self.Qsa[row][col][legal_moves]

        if choice > self.epsilon or q_learn:
...
while env.games > 0:
    ...
    if done:
        target = r
        agent.Qsa[curr_row][curr_col][act] += 0.1 * (target - agent.Qsa[curr_row][curr_col][act])
        ...
        next_string_pos = f"{next_state[0]}{next_state[1]}"
        next_legal_moves = env.legit_actions[next_string_pos]

        # next_act = agent.act(next_state, next_legal_moves, True)
        # target = r + agent.gamma * agent.Qsa[next_state[0]][next_state[1]][next_act]

        max_next_q = np.max(agent.Qsa[next_state[0]][next_state[1]][next_legal_moves])
